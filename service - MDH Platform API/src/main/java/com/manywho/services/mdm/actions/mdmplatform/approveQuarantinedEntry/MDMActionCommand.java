...
public class MDMActionCommand implements ActionCommand<ServiceConfiguration, MDMAction, MDMAction.Inputs, MDMAction.Outputs>{
	...
	public ActionResponse<MDMAction.Outputs> execute(ServiceConfiguration configuration, ServiceRequest request,
		...
		String resource = String.format("%s/universes/%s/quarantine/sources/%s/entities/%s/approve", input.getRepositoryID(), input.getUniverseID(), input.getSourceId(), input.getEntityId());
		Document document = AtomsphereAPI.executeAPIXML(configuration, "repositories", "POST", resource, null);

		return new ActionResponse<>(new MDMAction.Outputs(new ApproveResponse(document)));
	}
}
